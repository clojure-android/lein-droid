...
(defn extract-resources-from-jar
  ...
  `out-dir`."
  [jar-file out-dir]
  (with-open [zis (ZipInputStream. (io/input-stream jar-file))]
    (let [res-dir (str "res" File/separator)]
        ...
        (if-let [entry (.getNextEntry zis)]
          (if (and (.startsWith (str entry) res-dir)
            ...
            (let [out-file (io/file out-dir (str entry))]
              ...
              (with-open [out (io/make-output-stream out-file {})]
                (read-binary-file zis out))
              (recur true))
            (recur found-res))
          (when found-res
